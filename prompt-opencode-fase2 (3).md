...
### 6.1 Migración de usuarios técnicos, usuarios clientes y casillas de correo

Además de los tickets, hay que migrar las cuentas que los usan — sin esto, los tickets migrados quedan sin técnicos reales asignados ni clientes que puedan verlos en el portal.

**Descubrimiento de contraseñas (ya confirmado):** Zammad usa **Argon2id** para hashear contraseñas (`$argon2id$v=19$...`), incompatible con el `bcrypt` que usa SolidOps hoy. **No se pueden migrar los hashes directamente.**

**Decisión: migración perezosa (lazy migration), sin resetear contraseñas de nadie.**
1. Al migrar un usuario, guardá su hash de Argon2 original en un campo nuevo `legacy_argon2_hash` (nullable), y dejá `password_hash` (el de bcrypt) vacío por ahora.
2. En el login (tanto de staff como del portal de clientes), si `password_hash` está vacío pero `legacy_argon2_hash` existe: verificá la contraseña ingresada contra el hash de Argon2 (usá una librería de Node compatible, ej. `argon2` o `node-argon2`, en modo verify-only). Si coincide, **generá el hash bcrypt nuevo con esa misma contraseña en ese momento, guardalo en `password_hash`, y borrá `legacy_argon2_hash`** — a partir de ahí ese usuario ya está en bcrypt de forma transparente, sin que haya notado nada.
3. Si la verificación contra Argon2 falla (contraseña incorrecta) o si `legacy_argon2_hash` es null y `password_hash` también está vacío (usuario nunca migrado o sin contraseña en origen), tratalo como login fallido normal — no reveles cuál de los dos casos es, por seguridad.

**Mapeo de roles Zammad → SolidOps (confirmado):**
- Zammad `Admin` → SolidOps `Administrador`
- Zammad `Agent` → SolidOps `Técnico` (el supervisor puede reasignar manualmente a Supervisor/Coordinador después, caso por caso, desde la interfaz — no hay forma automática de saber quién debería ser cuál)
- Zammad `Customer` → `Contact` con `portal_enabled: true`, asociado a su `Customer`/empresa correspondiente

**Qué migrar de usuarios técnicos (Zammad `Agent`/`Admin`):** nombre, email, rol mapeado, `legacy_argon2_hash`, estado activo/inactivo. Crear también su `Technician` asociado (especialidad/nivel quedan vacíos para completar manualmente después, Zammad no tiene ese dato).

**Orden de migración obligatorio (no asumir que el `Customer` ya existe):** primero migrá las **organizaciones de Zammad** (`Organization`) como `Customer` en SolidOps de forma independiente — todas las que existan, tengan tickets migrados en L1/L2/L3 o no. Recién después migrá los usuarios cliente (`Contact`), asociándolos a esas organizaciones. Esto evita perder clientes que tengan cuenta de portal habilitada pero ningún ticket en los grupos migrados (por ejemplo, un cliente nuevo recién dado de alta, o alguien que solo escribió a los grupos de ruido que se descartan).

**Qué migrar de usuarios clientes (Zammad `Customer`):** nombre, email, teléfono si existe, `legacy_argon2_hash`, asociados al `Customer`/`Contact` correspondiente (mismo matcheo por email que ya se usa para los tickets).

**Qué migrar de casillas de correo:** los 3 `Channel::Email` ya relevados (`mesadeayuda@solidocs.com.ar`, `mkbackups@solidocs.com.ar`, `soporte@solidocs.com.ar`) → cargar como `Mailbox` en SolidOps usando el módulo que ya existe (sección 1.1) — esto es carga de datos, no requiere código nuevo. Las contraseñas de estas casillas de email (IMAP/SMTP) **no tienen el mismo problema que las de usuario** — se migran tal cual, cifradas con el mecanismo que ya existe.

Extendé el script de exportación (`import/export_zammad.rb`) y el importador para cubrir usuarios/contactos/casillas, en el mismo formato de archivo JSON o en archivos separados, lo que te resulte más prolijo — documentá cuál elegiste. No corras esta migración de usuarios/casillas sin los mismos backups obligatorios ya definidos para los tickets.
